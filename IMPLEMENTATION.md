...
2. 活跃判定：窗口 focused + 当前 tab active + 当前页为普通页面 + 系统非空闲（`chrome.idle`：无键鼠输入约 1 分钟即视为离开；有声页面 `tab.audible` 不判空闲，看视频/听音乐不受影响）。
3. 单一计时器（对应 DESIGN.md 第 12 节问题 A）：状态切换时先结算旧状态，再决定是否开启新计时；保证同一时刻只有一个计时区间。
4. 防休眠误计（问题 C）：结算时若区间时差异常大（阈值 `MAX_SESSION_MS = 30 分钟`），丢弃该段。理由：正常使用中 alarm 每 1 分钟结算一次，单区间几乎不超过 2 分钟；30 分钟阈值用于容忍 Chrome 对 alarms 的后台节流延迟（此前 5 分钟阈值会把延迟导致的正常浏览误判为睡眠整段丢弃），而真实睡眠（数小时）远超阈值仍会被正确丢弃。
5. 持久化（问题 B）：计时开始记录时间戳，结算时累加 `usageSeconds` 并立即写入 `chrome.storage.local`；不得只存在内存变量。**计时区间开始时间同样持久化（`timingSession` 键）**：SW 被回收/重启后结算优先使用持久化时间戳，杜绝区间丢失。
6. 用 `chrome.alarms` 做周期结算（如每分钟），防止 Service Worker 被回收时丢失未结算时长；**alarm 创建需幂等**（启动时先 `get` 检查再 `create`），避免 SW 每次唤醒重置周期。
7. 本阶段添加权限（每项说明理由）：`tabs`（后台读取当前 tab 的 URL 与 title 判断计时）、`alarms`（周期结算兜底）、`idle`（检测系统无输入，人离开电脑时停止计时）。
